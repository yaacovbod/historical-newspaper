...
export default function ArticleTypeSelector({ onSelect }: Props) {
  return (
    <div className="space-y-6">
      <h2 style={{ fontFamily: 'inherit', fontSize: '1.4rem', color: '#2c1810' }}>בחר סוג כתבה</h2>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {OPTIONS.map(opt => (
          ...
          >
            <div style={{ fontFamily: 'inherit', fontWeight: 700, color: '#2c1810', fontSize: '1rem', marginBottom: '0.4rem' }}>{opt.title}</div>
            <div style={{ color: '#5c3d1e', fontSize: '0.85rem', fontFamily: 'inherit' }}>{opt.description}</div>
          </button>
        ))}
      </div>
    </div>
